...
export const getProjectsReport = async (req, res, next) => {
    try {
        const userId = req.user.id;
...
export const getFreelancersReport = async (req, res, next) => {
    try {
...
export const exportReport = async (req, res, next) => {
    try {
        return res.json(successResponse('Export ready', { url: `/mock-exports/report-${req.user.id}.csv` }));
    }
    catch (error) {
        next(error);
    }
