...
export const uploadFile = async (req, res, next) => {
    try {
        ...
        if (String(category || '') === 'project_attachment' &&
            getMimeCategory(file.mimetype) === 'image') {
            try {
                fs.unlinkSync(file.path);
            }
            catch { /* ignore */ }
            return res.status(400).json(errorResponse('Images are not allowed for project attachments', 'VALIDATION_ERROR'));
        }
...
export const listFiles = async (req, res, next) => {
    try {
        ...
        const where = { uploadedBy: req.user.id, deletedAt: null, status: 'active' };
        if (category)
...
export const renameFile = async (req, res, next) => {
    try {
        const { name } = req.body;
        if (!name)
            return res.status(400).json(errorResponse('New name is required', 'VALIDATION_ERROR'));
        const file = await prisma.mediaFile.findFirst({
            where: { id: req.params.id, uploadedBy: req.user.id, deletedAt: null, status: 'active' }
        });
        if (!file)
...
export const restoreFile = async (req, res, next) => {
    try {
        ...
        if (!file)
            return res.status(404).json(errorResponse('File not found in trash', 'NOT_FOUND'));
        await prisma.mediaFile.update({
            where: { id: file.id },
