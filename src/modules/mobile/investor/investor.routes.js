...
import { getWallet, getTransactions, requestWithdrawal } from './controllers/wallet.controller.js';
...
import { getWatchlist as getWatchlistFounder, addToWatchlist as addToWatchlistFounder, removeFromWatchlist as removeFromWatchlistFounder } from '../founder/controllers/watchlist.controller.js';
const router = Router();
router.use(authenticate);
router.get('/watchlist', (req, res, next) => {
    if (req.user.role === 'founder') {
        return getWatchlistFounder(req, res, next);
    }
    return getWatchlist(req, res, next);
});
router.post('/watchlist', (req, res, next) => {
    if (req.user.role === 'founder') {
        return addToWatchlistFounder(req, res, next);
    }
    return addToWatchlist(req, res, next);
});
router.delete('/watchlist/:id', (req, res, next) => {
    if (req.user.role === 'founder') {
        return removeFromWatchlistFounder(req, res, next);
    }
