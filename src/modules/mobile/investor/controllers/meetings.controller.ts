...
export const rescheduleMeeting = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    await prisma.meeting.updateMany({ where: { id: req.params.id, investor: req.user.id }, data: { date, time } });
...
export const cancelMeeting = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    await prisma.meeting.updateMany({ where: { id: req.params.id, investor: req.user.id }, data: { status: 'Cancelled' } });
