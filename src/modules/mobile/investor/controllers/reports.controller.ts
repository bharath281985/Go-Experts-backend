...
export const getRoiReport = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try { return res.json(successResponse('ROI report', { overallROI: 0, byStartup: [] })); } catch (error) { next(error); }
};

export const getIndustryReport = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try { return res.json(successResponse('Industry report', { industrySplit: [] })); } catch (error) { next(error); }
};
