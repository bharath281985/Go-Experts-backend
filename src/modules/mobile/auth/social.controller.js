...
import { verifyAppleIdToken, verifyGoogleIdToken, } from '../../../services/mobile/social-auth.service.js';
...
const issueAuthResponse = async (user, deviceId, fcmToken, platform, deviceName) => {
    if (fcmToken) {
...
const findOrCreateSocialUser = async (email, fullName, role, avatarUrl) => {
...
const mapSocialError = (error, provider) => {
    if (!(error instanceof Error))
        return null;
    switch (error.message) {
        case 'INVALID_GOOGLE_TOKEN':
        case 'INVALID_APPLE_TOKEN':
            return errorResponse(`Invalid ${provider === 'google' ? 'Google' : 'Apple'} token`, 'SOCIAL_AUTH_FAILED');
        case 'GOOGLE_EMAIL_UNAVAILABLE':
        case 'APPLE_EMAIL_UNAVAILABLE':
            return errorResponse(`${provider === 'google' ? 'Google' : 'Apple'} account email not available`, 'SOCIAL_AUTH_FAILED');
...
export const googleSocialLogin = async (req, res, next) => {
    try {
        ...
        if (roleError)
            return res.status(422).json(roleError);
        if (!idToken) {
            return res.status(422).json(errorResponse('Google token is required', 'VALIDATION_ERROR'));
        }
        const identity = await verifyGoogleIdToken(idToken);
        const user = await findOrCreateSocialUser(identity.email || email, fullName || identity.fullName || identity.email.split('@')[0], role, identity.picture);
        ...
        if (mapped) {
            const status = error.message === 'ACCOUNT_INACTIVE' ? 403 : 401;
            return res.status(status).json(mapped);
        }
        next(error);
    }
};
export const appleSocialLogin = async (req, res, next) => {
    try {
        ...
        const user = await findOrCreateSocialUser(identity.email, fullName || identity.fullName || 'Apple User', role);
        const tokens = await issueAuthResponse(user, deviceId, fcmToken, platform, deviceName);
        await AuditEngine.track(user.id, 'social_login', 'user', user.id, null, null, req);
