...
import { assertGatewayAllowed, buildProductInfo, parseProductInfo, } from '../../../utils/payment-gateways.js';
import { loadPaymentMeta, storePaymentMeta } from '../../../utils/payment-meta.js';
export const initiatePaymentService = async (userId, gateway, amount, currency, metadata = {}) => {
...
export const verifyPaymentService = async (userId, paymentId, gateway, verification) => {
    ...
    if (gateway === 'easebuzz') {
        ...
        const productinfo = String(verification.productinfo || meta.productinfo || '');
        if (hash) {
            verified = verifyEasebuzzReverseHash(txnid, amount, status, hash, email, firstname, productinfo);
        }
    ...
    if (purpose === 'subscription' && planId) {
        const billingCycle = String(verification.billingCycle || meta.billingCycle || 'monthly').toLowerCase() ===
            'yearly'
            ? 'yearly'
            : 'monthly';
...
export const completePaymentFromWebhook = async (txnid, productinfo) => {
    const payment = await prisma.payment.findFirst({
    ...
    if (purpose === 'subscription' && planId) {
        await activateUserSubscription(payment.userId, planId, parsed.billingCycle ||
