...
export const getTransactions = async (req, res, next) => {
    try {
        ...
        try {
            ...
            try {
                const rawTx = await prisma.$queryRawUnsafe(
...
export const getCredits = async (req, res, next) => {
    try {
        const wallet = await prisma.wallet.findUnique({ where: { userId: req.user.id } });
        if (!wallet)
            return res.json(successResponse('Credits retrieved', []));
...
export const getDebits = async (req, res, next) => {
    try {
        const wallet = await prisma.wallet.findUnique({ where: { userId: req.user.id } });
        if (!wallet)
            return res.json(successResponse('Debits retrieved', []));
...
export const getPendingPayouts = async (req, res, next) => res.json(successResponse('Pending payouts', []));
export const getPaymentHistory = async (req, res, next) => res.json(successResponse('Payment history', []));
...
export const requestWithdrawal = async (req, res, next) => {
    try {
        ...
        if (wallet.balance < parsedAmount) {
            return res.status(400).json(errorResponse(`Insufficient balance. Available: ₹${wallet.balance.toFixed(2)}`, 'INSUFFICIENT_BALANCE'));
        }
