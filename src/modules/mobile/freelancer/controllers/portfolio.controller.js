...
const portfolioKey = (userId) => `freelancer_portfolio:${userId}`;
...
const writeItems = async (userId, items) => {
...
export const listPortfolio = async (req, res, next) => {
    try {
        ...
        if (search) {
            items = items.filter((item) => [item.title, item.description, item.projectUrl, ...(item.technologies || [])]
                .filter(Boolean)
                .join(' ')
                .toLowerCase()
                .includes(search));
        ...
        return res.json(successResponse('Portfolio retrieved', data, {
            page,
            limit,
            total,
