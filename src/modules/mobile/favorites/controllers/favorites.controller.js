...
const favKey = (userId) => `favorites:${userId}`;
...
const saveFavorites = async (userId, items) => {
...
export const addFavorite = async (req, res, next) => {
    try {
        ...
        const existing = items.find((i) => i.entityType === entityType && i.entityId === String(entityId));
        if (existing) {
...
const populateFavorites = async (items) => {
...
export const listFavorites = async (req, res, next) => {
    try {
        ...
        if (entityType)
            items = items.filter((i) => i.entityType === entityType);
        const total = items.length;
        const skip = (page - 1) * limit;
        const slice = items.slice(skip, skip + limit);
        const populated = await populateFavorites(slice);
        return res.json(successResponse('Favorites', populated, {
            page,
            limit,
            total,
...
export const getFavorite = async (req, res, next) => {
    try {
        ...
        const items = await loadFavorites(req.user.id);
        const item = items.find((i) => i.id === id || i.entityId === id);
        if (!item) {
            return res.status(404).json(errorResponse('Favorite not found', 'NOT_FOUND'));
        }
        const [populated] = await populateFavorites([item]);
        return res.json(successResponse('Favorite details', populated));
    }
    catch (error) {
        next(error);
    }
...
export const removeFavorite = async (req, res, next) => {
    try {
        const id = req.params.id;
        const items = await loadFavorites(req.user.id);
        const nextItems = items.filter((i) => i.id !== id && !(i.entityType + ':' + i.entityId === id));
        // Also allow delete by entityType:entityId composite
        const { entityType, entityId } = req.query;
        const filtered = entityType && entityId
            ? items.filter((i) => !(i.entityType === entityType && i.entityId === String(entityId)))
            : nextItems;
...
export const toggleFavorite = async (req, res, next) => {
    try {
        ...
        const items = await loadFavorites(req.user.id);
        const idx = items.findIndex((i) => i.entityType === entityType && i.entityId === String(entityId));
        if (idx >= 0) {
            items.splice(idx, 1);
            await saveFavorites(req.user.id, items);
