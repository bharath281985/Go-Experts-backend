...
async function resolveCheckoutUserId(req: AuthenticatedRequest, bodyUserId?: string): Promise<string> {
  const candidate = bodyUserId || req.user?.id;
  if (!candidate) throw new Error("userId required");
  const user = await prisma.user.findFirst({ where: { id: candidate, deletedAt: null } });
  if (!user) throw new Error("Portal user not found for payment (userId must reference users table)");
  return user.id;
}

// POST /checkout — auth required
router.post("/checkout", authMiddleware as any, async (req: AuthenticatedRequest, res: Response) => {
  try {
  ...
  try {
    ...
    if (txnid) {
      ...
      if (payment) {
        ...
        if (["success", "successful", "completed", "paid"].includes(statusRaw)) status = "completed";
